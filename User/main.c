...
void GetPWM(){
	if(TM_DELAY_Time() - timer2 >= 50){
		  ...
		  if (Tz == 1000 && Tr == 0 && Tp == 0 && Ty == 0)
			{
				w1 = 1000;w2 = 1000;w3 = 1000;w4 = 1000;
			}
			else
			{
				//w1 = Tz/(4*KT) - Ty/(4*KQ) + Tp/(2*KT*L);
				//w2 = Ty/(4*KQ) + Tz/(4*KT) + Tr/(2*KT*L);
				//w3 = Tz/(4*KT) - Ty/(4*KQ) - Tp/(2*KT*L);
				//w4 = Ty/(4*KQ) + Tz/(4*KT) - Tr/(2*KT*L);
				
				w1 = Ty/(4*KQ) + Tz/(4*KT) + Tr/(2*KT*L);
				w2 = Tz/(4*KT) - Ty/(4*KQ) + Tp/(2*KT*L);
				w3 = Tz/(4*KT) - Ty/(4*KQ) - Tp/(2*KT*L);
				w4 = Ty/(4*KQ) + Tz/(4*KT) - Tr/(2*KT*L);
			...
			TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_1, w2); //BRUSHLESS PA5  pitch
		TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_2, w4); //BRUSHLESS PA1 roll
			TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_3, w3); //BRUSHLESS PA2 pitc
		TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_4, w1); //BRUSHLESS PA3 roll
...
void SendGCS(int waktu){
				...
				TM_USART_Send(USART1, dataYaw_1.bytes, 4);
				TM_USART_Send(USART1, dataPitch_1.bytes, 4);
				TM_USART_Send(USART1, dataRoll_1.bytes, 4);
				...
				*/	
				//sprintf(buffer, "%d,%.2f\n",rawADC,baterai.value);
						//sprintf(buffer,"%.2f,%.2f,%.2f || %.2f,%.2f,%.2f || %.2f,%.2f,%.2f \n\r", Ty,Tr,Tp, dataYaw,dataPitch,dataRoll, error1, error2, error3);
			//sprintf(buffer,"%f,%f,%f,||%f,%.2f,%.2f\n\r",currentLat.value,currentLong.value,currentLat1.value1,currentLong1.value1,dataPitch,dataRoll);	
				sprintf(buffer, "\n\rR:%.2f,ErrR:%.2f,  PidR:%.2f, M1R:%d, M2R:%d, || P:%.2f, ErrP:%.2f, PidP:%.2f, M1P:%d, M2P:%d || Y:%.2f, Set:%.2f,  ErrY:%.2f, PidY:%.2f,",	
				dataRoll,error2, pidr, OutpidP2, OutpidP4, dataPitch, error1, pidp, OutpidP1, OutpidP3, dataYaw, dataYaw_Zero-dataYaw, error3, pidy);
...
int main(void){
	...
	TM_ADC_Init(ADC1, ADC_Channel_14); //Initialize Adc_Channel_14 PIN:PC4
	...
	// KALIBRASI IMU
	KalibrasiIMU();
			TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_1, 1000);		//PA5		pitch
			TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_3, 1000);		//PA2		pitch
		TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_2, 1000); 		//PA1			roll
		TM_PWM_SetChannelMicros(&TIM2_Data, TM_PWM_Channel_4, 1000); 		//PA3			roll
	//########>>>>>>>>START<<<<<<<########//
